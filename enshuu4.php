...
<body>
  ...
  foreach ($holidays as $key => $val) {
    ...
    if ($mm_key == $mm) {
      ...
      $shukujitus[] = "$mm_key/$dd_key $val<br>";
      if (date('w', strtotime(sprintf("%04d", $year) . sprintf("%04d", $key))) == 0) {
        $furikaeKyuujitu = true;
        while (is_holiday($holidays, $mm_key, $dd_key)) {
          $dd_key++;
        }
        $furikaeKyuujitu = false;
        $dd_key = sprintf("%02d", $dd_key);
        $dd_key = preg_replace('/^0+/', ' ', $dd_key);
        $shukujitus[] = "$mm_key/$dd_key 振替休日<br>";
      }
    }
  }
  print("<p>");
